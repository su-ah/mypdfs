...
\def\coursecode{S \& S}
\def\coursename{Intro to Measure Theory} % use \relax for non-course stuff
\def\studytype{1} % 1: Personal Self-Study Notes / 2: Course Lecture Notes / 3: Revised Notes / 4: Exercise Solution Sheet
\def\author{\me}
\def\createdate{May 13, 2025}
\def\updatedate{\today}
\def\source{Princeton Lectures in Analysis III} % name, ed. of textbook, or `Class Lectures` for class notes
\def\sourceauthor{Elias M. Stein \& Rami Shakarchi} % for class notes, put lecturer
