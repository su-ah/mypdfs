...
\begin{exr}
	...
	is left/right noetherian/artinian. %TASK
\end{exr} \
...
\begin{pf}
	...
	On the other hand, $\bZ$ is not artinian,
	so the ring is not artinian.
\end{pf} \
...
\nq
\begin{exr}[title=Exercise 1.4(c)] %HACK
...
\begin{exr}[title=Exercise 1.4*] %HACK
	Let $a\in R$.
...
\begin{exr}[title=Exercise 1.5] %TODO
...
\begin{exr}[title=Exercise 1.6] %HACK
	Let $a,b\in R$.
...
\nq
\begin{exr}[title=Exercise 1.9] %HACK
...
\begin{exr}[title=Exercise 1.13 part c)] %TODO
...
\begin{pf}
	Since $R$ is algebraic, there exists non-zero $p\in k[x]$ such that
	$p(b)=0$. Note that:
	\begin{equation*}
	    p(b)=a_{n}b^{n}+ \dots + a_{1}b + a_{0} = 0 \ \implies \
		b\cdot\left(\frac{-1}{a_{0}}(a_{n}b^{n-1}+\dots +a_{2}b+a_{1})\right) = b\cdot b^{-1} = 1
	\end{equation*}
	by the same argument as in part b).
	By uniqueness of inverses, $b^{-1}\in B$ as needed.
	NOTE: a little unsure about this one
\end{pf}
