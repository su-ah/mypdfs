...
\begin{prop}[type=Exercise,num=1d)] %TODO
	Let us say that $M$ satisfies the \textbf{maximal condition} if every
	nonempty collection of submodules of $M$ has a maximal element.
	Prove that this is equivalent to noetherian-ness.
	Formulate/prove a version for artinian-ness.
\end{prop} \
...
\nq
\begin{exr}[title=Proposition 1.17] %NOTE
	Let $R,S$ be rings, and let $M$ be an $(R,S)$-bimodule.
	Let $T$ denote the ``triangular matrix ring'':
	\begin{equation*}
		T \ := \ \begin{bmatrix} R & M \\ 0 & S\end{bmatrix}
	\end{equation*}
    Prove that the left ideals of $T$ are of the form $I\oplus J$, where $J$ is a left ideal in $S$, and $I$ is a left $R$-module of $R\oplus M$ containing $MJ$.
\end{exr} \
...
\begin{exr}[title=Proposition 1.22] %NOTE
	Prove that $T$ is left noetherian if and only if $R$ and $S$ are
	left noetherian rings, and $M$ is a noetherian left $R$-module.
	(The same is true for artinian.)
\end{exr} \
...
\begin{exr}
	Determine whether the ring
	$\begin{bmatrix} \bQ & \bQ \\ 0 & \bZ \end{bmatrix}$
	is left/right noetherian/artinian.
\end{exr} \

\begin{pf}
	Note that $\bQ$ is a field, and thus necessarily noetherian.
	Furthermore, $\bZ$ is a PID, so any ascending chain of ideals
	corresponds to a decreasing sequence of positive integers,
	so $\bZ$ is also noetherian.
	Since $\bQ$ is 1-dimensional as a $\bQ$-vector space,
	by part b), the ring is thus noetherian.
	On the other hand, $\bZ$ is not artinian, so the ring is not artinian.
\end{pf} \

\begin{exr}
	Determine whether the ring
	$\begin{bmatrix} \bR & \bR \\ 0 & \bQ \end{bmatrix}$
	is left/right noetherian/artinian.
\end{exr} \

\begin{pf}
	Similarly to above, since $\bR$ and $\bQ$ are fields, they are both
	necessarily noetherian and artinian.
	Furthermore, $\bR$ is a 1-dimensional vector space over itself, so
	the ring is indeed both noetherian and artinian.
\end{pf}


\nq
\begin{exr}[title=Example 1.25] %NOTE
	Let $\Delta$ be a division ring, and let $\sigma:\Delta\rightarrow\Delta$ be an endomorphism
	which is not an automorphism.
	Show that the skew-polynomial ring $\Delta[x;\sigma]$ is left noetherian
	but not right noetherian.
\end{exr} \
...
\begin{exr}[title=Example 1.26] %NOTE
	Show that the ring $\bZ\langle x,y\rangle /(y^2,yx)$ is noetherian on
...
\nq
\begin{exr}[title=Exercise 1.4(c)]
	Prove that if $R$ is a domain, then $R$ is Dedekind-finite.
\end{exr} \

\begin{pf}
	Suppose $ab=1$. Notice that $ba$ is then idempotent:
	\begin{equation*}
		(ba)^{2} \ = \ (ba)(ba) \ = \ b(ab)a \ = \ ba
	\end{equation*}
	Therefore:
	\begin{equation*}
		(ba)(1-ba) \ = \ ba-(ba)^{2} \ = \ ba-ba \ = \ 0
	\end{equation*}
	Since $R$ is a domain, $ab=1\neq0$ implies $a,b\neq0$.
	Thus, $ba\neq0$, so $1-ba=0$, or in particular, $ba=1$.
\end{pf} \

\begin{exr}[title=Exercise 1.4*]
	Let $a\in R$.
	Prove that if $a$ is left invertible, then it is not a left zero divisor.
	Prove that the converse holds if $a\in aRa$.
\end{exr} \

\begin{pf}
	Suppose $a$ is left invertible, and $ab=0$ for some $b$. Then:
	\begin{equation*}
		a^{-1}(ab) \ = \ (a^{-1}a)b \ = \ b \ = \ 0
	\end{equation*}
	Thus, it cannot be a left zero divisor.

	On the other hand, if $a\in aRa$, then $a=aba$ for some nonzero $b\in R$.
	Since $a$ is not a left zero divisor, then $ab\neq0$, so by definition,
	$a=(ab)a$ implies $ab$ is a left inverse of $a$.
\end{pf} \

\begin{exr}[title=Exercise 1.5]
	Give an example of a ring $R$ containing an element $x\in R$ such that
	$Rx\subsetneq xR$.
\end{exr} \

\begin{pf}
	Consider $\bZ[x]$, and define $\sigma_{x}:\bZ[x]\gto\bZ[x]$ as the ring homomorphism:
	\begin{equation*}
		\sigma_{x}(p) \ = \ xp
	\end{equation*}
	Then, consider the skew polynomial ring $R=\bZ[x;\sigma]$, with
	multiplication given by:
	\begin{equation*}
		ab \ = \ \sigma_{x}(b)a \ = \ xba
	\end{equation*}
	Then, for any $r\in R$, we have that:
	\begin{equation*}
		rx \ = \ \sigma_{x}xr \ = \ xxr
	\end{equation*}
	So evidently, $Rx\subseteq xR$.
	To see it is a strict subset, consider $x(x+1)\in xR$.
	If $x(x+1)\in Rx$, then there would exist some $r\in R$ such that
	$x(x+1) = rx = \sigma(x)r = xxr$.
	But clearly $x+1\neq xr$ for any $r$.
\end{pf} \

\begin{exr}[title=Exercise 1.6]
	Let $a,b\in R$.
	Prove that if $1-ab$ is left invertible, then so is $1-ba$.
\end{exr} \

\begin{pf}
	Suppose $u(1-ab)=1$ for some $u\in R$. Then:
	\begin{equation*}
		(1+bua)(1-ba)
		\ = \ 1-ba+bua-buaba
		\ = \ 1-ba+bu(1-ab)a
		...
		\ = \ 1
	\end{equation*}
	Thus $1-ba$ is left invertible as well.
\end{pf}


\nq
\begin{exr}[title=Exercise 1.7 part a)]
...
\newpage
\begin{exr}[title=Exercise 1.7 part b)]
...
\begin{exr}[title=Exercise 1.7 part c)]
...
\nq
\begin{exr}[title=Exercise 1.9]
	Let $R$ be any ring and let $n\geq 1$.
	Show that $Z(M_n(R)) = Z(R)\cdot I_n$.
	That is, show that every matrix in the center of $M_n(R)$ must be
	a scalar multiple of the identity, and that
	this scalar must be a central element in $R$.
\end{exr} \

\begin{pf}
	The reverse inclusion is evident, so suppose $A\in Z(M_{n}(R))$.

	Let $E_{ij}$ denote the matrix which is 0 except for the $ij$-th entry,
	which is 1. Then:
	\begin{gather*}
		(AE_{ij})_{rq} \ = \ \sum_{k=1}^{n}A_{rk}(E_{ij})_{kq}
		\ = \ \sum_{k=1}^{n}A_{rk}\delta_{ik}\delta_{jq}
		\ = \ A_{ri}\delta_{jq} \\
		(E_{ij}A)_{rq} \ = \ \sum_{k=1}^{n}(E_{ij})_{rk}A_{kq}
		\ = \ \sum_{k=1}^{n}A_{kq}\delta_{ri}\delta_{kj}
		\ = \ A_{jq}\delta_{ri}
	\end{gather*}
	So $AE_{ij}$ has as its $j$-th column the $i$-th column of $A$, and
	$E_{ij}A$ has as its $i$-th row the $j$-th row of $A$.
	The remaining entries are all 0. Notice:
	\begin{equation*}
		(AE_{ij})_{ij} \ = \ A_{ii} \qquad
		(E_{ij}A)_{ij} \ = \ A_{jj}
	\end{equation*}
	Since $A$ is central, $AE_{ij}=E_{ij}A$, so $A_{ii}=A_{jj}$.
	In particular, all diagonal entries of $A$ are the same.

	To see that it is diagonal, consider $E_{ii}$ for any $i$.
	Then, $AE_{ii}$ sets all but the $i$-th column to 0,
	while $E_{ii}A$ sets all but the $i$-th row to 0.
	Since $A$ is central, this implies that every non-diagonal entry must be 0.
	Since this holds for every $i$, it follows that $A$ must be diagonal.

	Therefore, we have that $A=rI$ for some $r\in R$.
	It remains to show that $r\in Z(R)$.
	Indeed, for any $a\in R$, consider $aI$. Then:
	\begin{equation*}
		(rI)(aI) \ = \ (ra)I \ = \ (aI)(rI) \ = \ (ar)I
	\end{equation*}
	In particular, $ra=ar$ for all $a\in R$.
	Thus, $r\in Z(R)$ as needed.
\end{pf}


\nq
\begin{exr}[title=Exercise 1.12 part a)]
...
\begin{exr}[title=Exercise 1.12 part b)]
...
\nq
\begin{exr}[title=Exercise 1.13 part a)]
	Show that if $R$ is finite-dimensional as a $k$-vector space,
	$R$ is algebraic. Is the converse true?
\end{exr} \
...
\begin{pf}
	...
	Consider the following field extension:
	\begin{equation*}
		R \ := \ \bQ[\sqrt{2},\sqrt[3]{2},\sqrt[4]{2},\dots]
	\end{equation*}
	That this is algebraic follows from the fact that any element in $R$
	is contained in the span of finitely many radicals, and that an algebraic
	extension of a field is algebraic.
	However, it is evidently not finite-dimensional as a $\bQ$-vector space.
\end{pf} \

\newpage
\begin{exr}[title=Exercise 1.13 part b)]
	Henceforth, let $R$ be an algebraic $k$-algebra.
	Show $R$ is Dedekind-finite.
\end{exr} \
...
\newpage
\begin{exr}[title=Exercise 1.13 part c)]
	Show that every left zero divisor is also a right zero divisor.
\end{exr} \

\begin{pf}
	Suppose $ab=0$ for some $b\neq0$.
	Then, there is some $p(x)\in k[x]$ with:
	\begin{equation*}
		p(x) \ = \ b_{n}x^{n} + \dots + b_{1}x + b_{0} \qquad p(b) \ = \ 0
	\end{equation*}
	Notice:
	\begin{equation*}
		0 \ = \ a\cdot p(b) \ = \ ab_{n}b^{n} + \dots + ab_{1}b + ab_{0}
		\ = \ b_{n}ab^{n} + \dots + b_{1}ab + ab_{0} \ = \ ab_{0}
	\end{equation*}
	where the $b_{i}\in k$ commute.
	In particular, $ab_{0}=b_{0}a=0$.
	If $b_{0}\neq0$, we're done, so suppose $b_{0}=0$. Then:
	\begin{equation*}
		0 \ = \ p(b)\cdot a \ = \ b_{n}b^{n}a + \dots + b_{1}ba
		\ = \ \left(\sum_{k=1}^{n}b_{i}\right)ba
	\end{equation*}
	Since $b\neq0$, then $p(x)\neq0$, so $\sum b_{i}\neq0$.
	In both cases, $a$ is a right zero divisor as needed.
\end{pf} \

\begin{exr}[title=Exercise 1.13 part d)]
	Suppose $r\in R$ is non-zero.
	Show that $r$ is a zero divisor iff it is not a unit.
\end{exr} \

\begin{pf}
	If $r$ is a unit, then $r$ cannot be a (left) zero divisor as:
	\begin{equation*}
	    rs = 0 \ \implies \ s = r^{-1}rs = r^{-1}\cdot0 = 0 \qquad
	    sr = 0 \ \implies \ s = srr^{-1} = 0\cdot r^{-1} = 0
	\end{equation*}
	On the other hand, if $r$ is not a (left) zero divisor, then the map
	$\vphi_{r}(s)=rs$ is injective. Thus, it has a (left-)inverse $\vphi$.
	Since $\vphi$ is necessarily surjective, $\vphi=\vphi_{a}$ for some $a\in R$.
	Then:
	\begin{equation*}
		(\vphi_{a}\circ\vphi_{r})(s) = ars = s \ \implies \ ar=1
	\end{equation*}
	So $r$ has a (left-)inverse.

	Since the sided-ness does not affect either argument, we are done.
\end{pf} \

\begin{exr}[title=Exercise 1.13 part e)]
	Let $B$ be a $k$-subalgebra of $R$, $b\in B$.
	Show that if $b$ is a unit in $R$, then $b^{-1}\in B$.
\end{exr} \

\begin{pf}
	Since $R$ is algebraic, there exists non-zero $p\in k[x]$ of minimal degree
	such that:
	\begin{equation*}
		p(b) \ = \ a_{n}b^{n}+ \dots + a_{1}b + a_{0} \ = \ 0
	\end{equation*}
	If $a_{0}=0$, then:
	\begin{equation*}
		p(b) \ = \ a_{n}b^{n} + \dots + a_{1}b
		\ = \ (a_{n}b^{n-1} + \dots  + a_{1})b
		\ = \ 0
	\end{equation*}
	implies by part d) that $q(x)=a_{n}x^{n-1} + \dots + a_{1}$ satisfies
	$q(b)=0$, but this contradicts minimality of $p$.
	Thus, if $a_{0}\neq0$:
	\begin{equation*}
	    p(b) \ = \ a_{n}b^{n}+ \dots + a_{1}b + a_{0} \ = \ 0
		\ \implies \
		\left(\frac{-1}{a_{0}}(a_{n}b^{n-1}+\dots +a_{2}b+a_{1})\right)\cdot b
		\ = \ 1
	\end{equation*}
	By uniqueness of inverses, $b^{-1}\in B$ as needed.
\end{pf}


\nq
\begin{exr}[title=Exercise 1.19] %TODO
	Let $D$ be a domain containing a minimal left ideal.
	Show that $D$ is a division ring.
	Conclude that every left artinian domain is a division ring;
	in particular a finite domain is a division ring.
\end{exr} \
