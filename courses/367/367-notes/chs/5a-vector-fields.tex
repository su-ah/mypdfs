...
whats a vector field? morally, a smooth choice of tanget vec at every pt in $M$.
...
\begin{defn}
	...
	for each chart $\vphi$, we have $D_{p}\vphi\circ X(p):U\gto\bR^{n}$ is smooth.
\end{defn}
this uses the identification $T_{p}M\simeq\bR^{n}$ induced by chart.
but what is that thing between $D_{p}\vphi\circ X(p)$?
